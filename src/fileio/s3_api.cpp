 ...
 */
std::string quote_and_esacpe_path(const std::string& path) {
    ...
    return std::async(std::launch::async,
                     [=]()->std::string{
                       ...
                       const std::vector<std::string> arglist{
                         "s3", "cp", "--recursive",
                         quote_and_esacpe_path(local_file),
                         quote_and_esacpe_path(remote_path.str()), S3_COMMAND_OPTION
                       };
    ...
    return std::async(std::launch::async,
                     [=]()->std::string{
                       ...
                       const std::vector<std::string> arglist{
                         "s3", "cp",
                         quote_and_esacpe_path(local_file),
                         quote_and_esacpe_path(remote_path.str()),
                         S3_COMMAND_OPTION
                       };
                       return graphlab::fileio::run_aws_command(arglist, access_key_id, secret_key);
                     });
