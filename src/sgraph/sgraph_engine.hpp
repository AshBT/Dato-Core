...
*/
#ifndef GRAPHLAB_SGRAPH_SGRAPH_sgraph_engine_HPP
#define GRAPHLAB_SGRAPH_SGRAPH_sgraph_engine_HPP
...
class sgraph_engine {
 ...
 private:
  ...
  graphlab::mutex lock_array[LOCK_ARRAY_SIZE];
    ...
    while (row_start < row_end) {
      ...
      for (const auto& edgedata : all_edgedata) {
        ...
        if (edgedir == edge_direction::IN_EDGE || 
          ...
          std::unique_lock<graphlab::mutex> guard(lock_array[vertexhash % LOCK_ARRAY_SIZE]);
          // perform the gather
