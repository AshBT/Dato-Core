...
bool sgraph::add_edges(sframe edges,
  ...
  commit_edge_buffer(groupa, groupb, edge_partitions);
...
void sgraph::commit_edge_buffer(size_t groupa,
  ...
  logstream(LOG_EMPH) << "In commit edge buffer (" << groupa << "," << groupb << ")"
                       << std::endl;
  ASSERT_EQ(edge_partitions.size(), m_num_partitions*m_num_partitions);
  ...
  logstream(LOG_EMPH) << "Aggregate unique vertices" << std::endl;
  ...
  sgraph_compute::hilbert_blocked_parallel_for(
      ...
      [&](std::pair<size_t, size_t> coordinate) {
        size_t i = coordinate.first;
        size_t j = coordinate.second;
        auto source_vid_buffer = source_vid_buffers[i];
        auto target_vid_buffer = target_vid_buffers[j];
        sframe& sf = edge_partitions[i * m_num_partitions + j];
        size_t src_column_idx = sf.column_index(SRC_COLUMN_NAME);
        size_t dst_column_idx = sf.column_index(DST_COLUMN_NAME);
        auto src_column = sf.select_column(src_column_idx);
        auto dst_column = sf.select_column(dst_column_idx);
        sarray_reader_buffer<flexible_type> src_reader(
            std::move(src_column->get_reader()),
            0, src_column->size());
        sarray_reader_buffer<flexible_type> dst_reader(
            std::move(dst_column->get_reader()),
            0, dst_column->size());
        while(src_reader.has_next()) {
          auto src = src_reader.next();
          if (src.get_type() == flex_type_enum::UNDEFINED) {
            std::string error_message =
              std::string("source vid column cannot contain missing value. ") +
              "Please use dropna() to drop the missing value from the input and try again";
            log_and_throw(error_message);
          }
          source_vid_buffer->add(std::move(src));
        }
        while(dst_reader.has_next()) {
          auto dst = dst_reader.next();
          if (dst.get_type() == flex_type_enum::UNDEFINED) {
            std::string error_message =
              std::string("target vid column cannot contain missing value. ") +
              "Please use dropna() to drop the missing value from the input and try again";
            log_and_throw(error_message);
          }
          target_vid_buffer->add(std::move(dst));
        }
      });
  parallel_for(0, vid_buffer.size(), [&](size_t i) { vid_buffer[i]->close();} );
  ...
  // write out the unique vertex ids for each partition, and merge with the vertex store.
  ...
  // for (size_t i = 0; i < vid_buffer.size(); ++i) {
    ...
    logstream(LOG_INFO) << "Sort and write unique vertex data in partition " << partitionid << std::endl;

    // sort and write guarantee new_vids are unique.
    std::vector<flexible_type> new_vids;
    new_vids.reserve(vid_buffer[i]->approx_size());
    vid_buffer[i]->sort_and_write(std::inserter(new_vids, new_vids.begin()));
    logstream(LOG_INFO) << "Finish sort and write vertex data in partition " << partitionid << " in " << timer.current_time() << " secs" << std::endl;

    timer.start();
    logstream(LOG_INFO) << "Writing new vertices in partition " << partitionid << " size = " << new_vids.size() << std::endl;
    // fetch existing vids and create a set.
    std::vector<flexible_type> old_vids = get_vertex_ids(partitionid, groupid);
    std::unordered_set<flexible_type> old_vid_set(std::make_move_iterator(old_vids.begin()),
                                                  std::make_move_iterator(old_vids.end()));

    // write new vertices into a temporary sframe.
    sarray<flexible_type> new_vid_sarray;
    new_vid_sarray.open_for_write(1);
    new_vid_sarray.set_type(m_vid_type);
    auto out = new_vid_sarray.get_output_iterator(0);
    size_t new_vertices_cnt = 0;
    for (auto& id: new_vids) {
      if (!old_vid_set.count(id)) {
        *out = std::move(id);
        ++out;
        ++new_vertices_cnt;
      }
    }
    new_vid_sarray.close();

    sframe new_vertices;
    vertices_added += new_vertices_cnt;
    new_vertices = new_vertices.add_column(std::make_shared<sarray<flexible_type>>(new_vid_sarray),
                                           VID_COLUMN_NAME);
    logstream(LOG_INFO) << "Finish writing new vertices in partition " << partitionid << " in " << timer.current_time() << " secs" << std::endl;
    ...
    // debug print
    // std::cerr << " Vertices in partition " << i << ":\n";
    // old_vertices.debug_print();
  // }
  });
  ...
  /*                                                                        */
  /**************************************************************************/
  logstream(LOG_EMPH) << "Rename id columns " << local_timer.current_time();
  ...
  std::function<void(std::vector<std::pair<size_t, size_t>>)>
    load_vid_hash_map_cache = [&](std::vector<std::pair<size_t, size_t>> coordinates) {
      ...
      parallel_for(0, block_to_load_vec.size(), [&](size_t i) {
          ...
          vid_hash_map_cache[coord] = std::move(fetch_vid_hash_map(coord.first, coord.second));
          });
    };
  ...
  sgraph_compute::hilbert_blocked_parallel_for(
      ...
      [&](std::pair<size_t, size_t> coordinate) {
        ...
        std::shared_ptr<std::unordered_map<flexible_type, size_t>>
  ...
  // debug print
  // std::cerr << " Edges in partition (" << i << ", " << j << "):\n";
  // old_edges.debug_print();
  vid_hash_map_cache.clear();
  logstream(LOG_EMPH) << "Done in " << local_timer.current_time() << " secs" << std::endl;
...
sgraph::fetch_vertex_data_field(std::string column_name, size_t groupid) {
  std::vector<std::shared_ptr<sarray<flexible_type>>> ret;
  auto& vgroups = vertex_group(groupid);
  for (size_t i = 0; i < vgroups.size(); ++i) {
 ...
 */
std::vector<std::shared_ptr<sarray<flexible_type>>> 
sgraph::fetch_edge_data_field(std::string column_name, size_t groupa, size_t groupb) {
  std::vector<std::shared_ptr<sarray<flexible_type>>> ret;
  auto& egroups = edge_group(groupa, groupb);
  for (size_t i = 0; i < egroups.size(); ++i) {
...
std::shared_ptr<sgraph::vid_hash_map_type> sgraph::fetch_vid_hash_map(size_t partition,
  ...
  std::vector<flexible_type> vids = get_vertex_ids(partition, group);
...
bool sgraph::union_columns(sframe& left, sframe& right) {
  ...
  return reorder_and_add_new_columns(left, names, types) &&
