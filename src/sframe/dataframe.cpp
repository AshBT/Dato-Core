...
/**************************************************************************/
/*                                                                        */
/*                     dataframe_inserter functions                      */
/*                                                                        */
/**************************************************************************/

dataframe_inserter::dataframe_inserter(dataframe_t& frame,
                                         const flexible_type_registry& registry):
    frame(frame), registry(registry) {
  nrows = 0;
  // initialize pointer to the frame values
  for (auto& tp : frame.values) {
    field_id_type fid = registry.get_field_id(tp.first);
    ASSERT_NE(fid, (field_id_type)(-1));
    field_to_column_index[fid] = values.size();
    values.push_back({fid, &tp.second});
    nrows = tp.second.size();
  }
}
