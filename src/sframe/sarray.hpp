...
*/
#ifndef GRAPHLAB_UNITY_SFRAME_SARRAY_HPP
#define GRAPHLAB_UNITY_SFRAME_SARRAY_HPP
#include <set>
#include <iterator>
#include <type_traits>
...
#include <util/function_output_iterator.hpp>
...
namespace swriter_impl {
  ...
  //
template <typename T>
using output_iterator = graphlab::function_output_iterator<
...
/**************************************************************************/
  ...
  void save(std::string index_file) const {
    ...
    new_sa.open_for_write(index_file, this->num_segments());

    // Make sure all segments are writable.
    for (size_t i = 0; i < num_segments(); ++i) {
      new_sa.get_output_iterator(i);
    }

    if (std::is_same<T, flexible_type>::value) {
      new_sa.set_type(get_type());
    }

    graphlab::copy_if(*this, new_sa, [](const value_type &f)->bool{return true;});

    new_sa.close();
  }

  void delete_files_on_destruction(bool del) {
...
BEGIN_OUT_OF_PLACE_SAVE(arc, std::shared_ptr<graphlab::sarray<graphlab::flexible_type> >, m) {
  if(m == nullptr) {
    arc << false;
  } else {
    arc << true;
...
BEGIN_OUT_OF_PLACE_LOAD(arc, std::shared_ptr<graphlab::sarray<graphlab::flexible_type> >, m) {
  bool is_not_nullptr;
  arc >> is_not_nullptr;
  if(is_not_nullptr) {
    m.reset(new graphlab::sarray<graphlab::flexible_type>);
    arc >> (*m);
  } else {
    m = std::shared_ptr<graphlab::sarray<graphlab::flexible_type> >(nullptr);
  }
