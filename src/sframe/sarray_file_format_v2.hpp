...
*/
#ifndef GRAPHLAB_SFRAME_SARRAY_FILE_FORMAT_V2_HPP
#define GRAPHLAB_SFRAME_SARRAY_FILE_FORMAT_V2_HPP
#include <string>
#include <memory>
#include <mutex>
#include <typeinfo>
...
class sarray_format_reader_v2: public sarray_format_reader<T> {
  ...
  void open(index_file_information index) {
    ...
    for (size_t i = 0;i < index.segment_files.size(); ++i) {
      ...
      for (size_t j = 0; j < nblocks; ++j) {
        ...
        row_count += m_manager.get_block_info(blockaddr).num_elem;
 ...
 private:
  ...
  void try_evict_something_from_cache() {
    size_t b = graphlab::random::fast_uniform<size_t>(0, m_cache.size() - 1);
    /*
    ...
    if (m_cache[b].lock.try_lock()) {
      release_cache(b);
      m_cache[b].lock.unlock();
    }
  }
...
template <typename T>
void sarray_group_format_writer_v2<T>::flush_block(size_t columnid, 
                                                   size_t segmentid){
