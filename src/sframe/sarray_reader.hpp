...
*/
#ifndef GRAPHLAB_UNITY_SFRAME_SARRAY_READER_HPP
#define GRAPHLAB_UNITY_SFRAME_SARRAY_READER_HPP
#include <set>
#include <iterator>
#include <type_traits>
#include <functional>
  ...
  void open_format_reader(const sarray<T>& array) {
    ...
    switch(file_version) {
     case 0:
       ...
       break;
     case 1:
       reader = new sarray_format_reader_v1<T>();
...
template <typename T>
inline int distance(const graphlab::sarray_iterator<T>& begin, 
                    const graphlab::sarray_iterator<T>& end) {
  return end - begin;
}
