...
/*                                                                          */
/****************************************************************************/
groupby_element::groupby_element(const std::vector<flexible_type>& group_key,
                                 const std::vector<group_descriptor>& group_desc) {
  init(group_key, group_desc);
}
...
void groupby_element::init(const std::vector<flexible_type>& group_key,
                           const std::vector<group_descriptor>& group_desc) {
  key = group_key;
...
bool groupby_element::flexible_type_vector_equality(const std::vector<flexible_type>& a,
...
bool groupby_element::flexible_type_vector_lt(const std::vector<flexible_type>& a,
                                              const std::vector<flexible_type>& b) {
...
void groupby_element::add_element(const std::vector<flexible_type>& val,
...
void group_aggregate_container::add(const std::vector<flexible_type>& val,
  ...
  std::unique_lock<graphlab::simple_spinlock> lock(segments[target_segment].in_memory_group_lock);
  ...
  for (size_t i = 0;i < groupby_element_vec->size(); ++i) {
    if (groupby_element::flexible_type_vector_equality((*groupby_element_vec)[i].key,
...
void group_aggregate_container::flush_segment(size_t segmentid) {
  ...
  std::unique_lock<graphlab::simple_spinlock> lock(segments[segmentid].in_memory_group_lock);
  ...
  std::unique_lock<graphlab::mutex> filelock(segments[segmentid].file_lock);
  oarchive oarc;
  for (auto& item: local_sorted) {
    oarc << item;
