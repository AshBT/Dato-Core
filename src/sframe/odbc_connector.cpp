...
void odbc_connector::handle_return(SQLRETURN orig_ret_status,
  ...
  do
  {
    ...
    if (SQL_SUCCEEDED(ret)) {
      odbc_err_msg << state << ":" << i << ":" << native << ":" << text;
      if(log_lvl == LOG_ERROR) {
...
size_t odbc_connector::calculate_batch_size(size_t max_row_size) {
  ...
  if(tmp_num_rows < 1) {
    ...
    err_msg << "The maximum size of one result row from this query ("
      << max_row_size << " bytes) will not fit in the allocated buffer ("
...
size_t odbc_connector::start_query(const std::string &query_str) {
  ...
  for(SQLSMALLINT i = 0; i < m_num_result_cols; ++i) {
    ...
    m_entry_buffer[i] = (SQLPOINTER)malloc(m_result_column_info[i].max_size_in_bytes * m_num_rows_to_fetch);
...
std::vector<std::vector<flexible_type>> odbc_connector::get_query_block() {
  ...
  for(SQLSMALLINT i = 0; i < m_num_result_cols; ++i) {
    ...
    for(SQLLEN j = 0; j < m_num_rows_fetched; ++j) {
      ...
      bool is_null = false;
      if(m_entry_buffer_size[i][j] == SQL_NULL_DATA) {
        ret_block[j][i] = FLEX_UNDEFINED;
        is_null = true;
      } else if(m_entry_buffer_size[i][j] == SQL_NO_TOTAL) {
        // I don't have time for stupid drivers that give me this. They get 8
        // bytes to say their piece.
        elem_len = 8;
      } else if(m_entry_buffer_size[i][j] >= 0) {
        elem_len = m_entry_buffer_size[i][j];
      } else {
        ret_block[j][i] = FLEX_UNDEFINED;
        is_null = true;
      }
      ...
      if(!is_null) {
        flexible_type out;
        this->sql_to_flexible_type(cur_data_pos, elem_len, c_type, out);
        ret_block[j][i] = out;
      }
...
void odbc_connector::sql_to_flexible_type(void *buf, size_t len, SQLSMALLINT sql_c_type, flexible_type &out) {
  if(sql_c_type == SQL_C_TYPE_TIMESTAMP) {
    ...
    out = flex_date_time(flexible_type_impl::my_to_time_t(the_time), 0);
...
std::pair<flexible_type, flexible_type> odbc_connector::get_column_limits(
  ...
  unity_sarray us;
  us.construct_from_sarray(column);
  bool is_max = true;
...
void odbc_connector::insert_data_impl(sframe &sf, const std::string &table_name, bool append_if_exists, bool auto_create_table) {
  ...
  if(row_bytes == 0) {
    //TODO: doesn't set back autocommit
