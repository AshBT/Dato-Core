...
void async_request_socket::keyval_change(graphlab::zookeeper_util::key_value* unused,
...
void* async_request_socket::get_socket(const size_t id) {
  ...
  if (targets[id].z_socket == NULL) {
    ...
    int rc = zmq_connect(targets[id].z_socket, targets[id].server.c_str());
    if (rc) {
      // unable to connect. Close the socket and fail
      std::cerr << "async_request_socket error: Unable to connect to " << targets[id].server
