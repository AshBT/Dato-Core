...
std::string get_local_ip_as_str(bool print) {
  ...
  if (ip == 0) return "127.0.0.1";
    ...
    assert(ip_conversion_success == true);
    return out;
  }
}
...
std::pair<size_t, int> get_free_tcp_port() {
  ...
  if (bind(sock, (sockaddr*)&my_addr, sizeof(my_addr)) < 0){
    std::cerr << "Failed to bind to a port 0! Unable to acquire a free TCP port!\n";
    assert(false);
  }
  ...
  if (getsockname(sock, &addr, &slen) < 0) {
    std::cerr << "Failed to get port information about bound socket\n";
    assert(false);
  }
