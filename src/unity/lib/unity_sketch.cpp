...
void unity_sketch::construct_from_sarray(
  ...
  std::set<flexible_type> key_set = std::set<flexible_type>(keys.begin(), keys.end());
         ...
         in_parallel([this, reader, key_set](size_t thr, size_t nthreads) {
               ...
               while (row_start < row_end) {
                if (m_cancel) break;
                 ...
                 std::unique_lock<graphlab::mutex> thrlocal_lock(m_thrlocks[thr]);
...
void unity_sketch::accumulate_one_value(size_t thr, const flexible_type& val, const std::set<flexible_type>& keys) {
...
void unity_sketch::accumulate_vector_value(const flexible_type& val, size_t thr, const std::set<flexible_type>& keys) {
  increase_nested_element_count(*m_element_sketch, thr, val.size());
...
void unity_sketch::accumulate_dict_value(const flexible_type& val, size_t thr, const std::set<flexible_type>& keys) {
  ...
  std::map<flexible_type, flexible_type> converted_dict;
...
void unity_sketch::init(
  ...
  flex_type_enum type,
  const std::set<flexible_type>& keys,
...
size_t unity_sketch::num_elements_processed() {
  std::unique_lock<graphlab::mutex> global_lock(lock);
...
unity_sketch::element_sub_sketch(const std::vector<flexible_type>& keys) {
  ...
  std::set<flexible_type> key_set(keys.begin(), keys.end());
...
void unity_sketch::combine_global(std::vector<graphlab::mutex>& thr_locks) {
  ...
  for (size_t i = 0;i < m_thrlocal.size(); ++i) {
    std::unique_lock<graphlab::mutex> thrlocal_lock(thr_locks[i]);
...
void unity_sketch::discrete_sketch_struct::reset() {
  ...
  frequent.reset(new sketches::space_saving<flexible_type>());
