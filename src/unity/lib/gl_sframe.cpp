...
/*                                                                        */
/**************************************************************************/
std::vector<flexible_type> gl_sframe::operator[](int i) {
...
std::vector<flexible_type> gl_sframe::operator[](int i) const {
  if (i < 0 || (size_t)i >= size()) {
    throw std::string("Index out of range");
  }
  if (!m_sframe_reader) {
...
gl_sframe gl_sframe::operator[](const std::initializer_list<int>& _slice) {
...
gl_sframe gl_sframe::operator[](const std::initializer_list<int>& _slice) const {
  std::vector<int> slice(_slice);
  long start = 0, step = 1, stop = 0;
  if (slice.size() == 2) {
    start = slice[0]; stop = slice[1];
...
const_gl_sarray_reference gl_sframe::operator[](const char* column) const {
  return const_gl_sarray_reference(*this, std::string(column));
}
...
gl_sarray_reference gl_sframe::operator[](const char* column) {
  return gl_sarray_reference(*this, std::string(column));
}
...
gl_sarray gl_sframe::apply(std::function<flexible_type(const std::vector<flexible_type>&)> fn,
                flex_type_enum dtype) const {
  return get_proxy()->transform_lambda(fn, dtype, false, time(NULL));
}
...
gl_sframe gl_sframe::sort(const std::map<std::string, bool>& column_and_ascending) const {
...
gl_sframe_range::gl_sframe_range(
  ...
  if (m_sframe_reader_buffer->has_next()) {
    m_current_value = std::move(m_sframe_reader_buffer->next());
  }
}
...
void gl_sframe_range::iterator::increment() {
  ...
  if (m_owner->m_sframe_reader_buffer->has_next()) {
    m_owner->m_current_value = std::move(m_owner->m_sframe_reader_buffer->next());
  }
}
