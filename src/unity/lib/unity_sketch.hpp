...
class unity_sketch: public unity_sketch_base {
 ...
 private:
  ...
  struct discrete_sketch_struct {
    ...
    std::shared_ptr<sketches::space_saving<flexible_type> > frequent;
  ...
  graphlab::atomic<size_t> m_rows_processed_by_threads;
  ...
  std::vector<graphlab::mutex> m_thrlocks;

  atomic<bool> m_cancel = false;
  std::future<void> m_background_future;
  graphlab::timer m_commit_timer;
  ...
  inline void init(
    unity_sketch* parent,
    flex_type_enum type,
    const std::set<flexible_type>& keys = std::set<flexible_type>(),
    std::shared_ptr<sarray<flexible_type>::reader_type> reader = NULL);

  inline void combine_global(std::vector<graphlab::mutex>& thr_locks);
  inline void accumulate_dict_value(const flexible_type& dict_val, size_t thr, const std::set<flexible_type>& keys);
  inline void accumulate_vector_value(const flexible_type& vect_val, size_t thr, const std::set<flexible_type>& keys);
  inline void accumulate_list_value(const flexible_type& rec_val, size_t thr);
  inline void accumulate_one_value(size_t thr, const flexible_type& val, const std::set<flexible_type>& keys = std::set<flexible_type>());
