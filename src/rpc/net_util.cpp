...
uint32_t get_local_ip(bool print) {
  ...
  if (c_subnet_id != NULL) {
    if (!str_to_ip(c_subnet_id, subnet_id)) {
      std::cerr << "Unable to convert GRAPHLAB_SUBNET_ID to a valid address. Cannot continue\n";
      exit(1); 
    }
  }
  if (c_subnet_mask != NULL) {
    if (!str_to_ip(c_subnet_mask, subnet_mask)) {
      std::cerr << "Unable to convert GRAPHLAB_SUBNET_MASK to a valid address. Cannot continue\n";
      exit(1); 
    }
  }
  ...
  if (c_subnet_id == NULL && c_subnet_mask != NULL) {
    ...
    std::cerr << "GRAPHLAB_SUBNET_MASK specified, but GRAPHLAB_SUBNET_ID not specified.\n";
    std::cerr << "We cannot continue\n";
    exit(1);
  } 
  if (c_subnet_id != NULL && c_subnet_mask == NULL) {
    if (print) {
      std::cerr << "GRAPHLAB_SUBNET_ID specified, but GRAPHLAB_SUBNET_MASK not specified.\n";
    ...
    if (print) {
      std::cerr << "GRAPHLAB_SUBNET_ID/GRAPHLAB_SUBNET_MASK environment variables not defined.\n";
