...
*/
#ifndef GRAPHLAB_UNITY_GRAPH_OPERATION_DAG_HPP
#define GRAPHLAB_UNITY_GRAPH_OPERATION_DAG_HPP
#include <functional>
#include <queue>
#include <vector>
#include <memory>
...
class lazy_eval_operation_dag {
 ...
 private:
  ...
  std::unordered_map<size_t, std::vector<size_t> > list_ancestors(size_t vertex) {
    ...
    graphlab::mutable_queue<size_t, size_t> vqueue;
...
std::ostream& operator<<(std::ostream& out, 
                         const graphlab::lazy_eval_operation_dag<T>& dag) {
  dag.print(out);
  return out;
}
