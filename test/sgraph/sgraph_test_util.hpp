 ...
 * Create a ring graph.
 */
inline sgraph create_and_check_ring_graph(size_t nverts, size_t npartition, bool bidirection=false) {
  sgraph g(npartition);
 ...
 * Create star graph.
 */
inline sgraph create_and_check_star_graph(size_t nverts, size_t npartition, bool bidirection=false) {
  sgraph g(npartition);
  std::vector<flexible_type> sources;
  std::vector<flexible_type> targets;
  std::vector<flexible_type> ids;
